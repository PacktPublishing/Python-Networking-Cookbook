...
url = f"{base_url}/api/fmc_config/v1/domain/{domain}/policy/accesspolicies/{policy_id}/accessrules/{rule_id}"
...
if resp.ok:
    ...
    if r.ok:
        ...
        print(f"Failed to update access rule. Status code: {r.status_code}")
else:
    print(f"Failed to retrieve access rule. Status code: {resp.status_code}")